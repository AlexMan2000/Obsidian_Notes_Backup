[Lab 10_ Priority Queues _ CS 61B Spring 2018.pdf](https://www.yuque.com/attachments/yuque/0/2023/pdf/12393765/1676300915929-aa088c4e-4ea8-48e2-956a-04024eed5835.pdf)
[lab10.zip](https://www.yuque.com/attachments/yuque/0/2023/zip/12393765/1676645622288-e02af4ae-82a5-4bf3-a20b-73d634c6009a.zip)

# Representing a Tree With an Array
> ![image.png](L1810__Heaps_Priority_Queues.assets/20230324_1601212011.png)
> 注意`ArrayHeap`只能代表`Complete Tree`, 读者可以自行思考为什么。
...
# Working With Binary Heaps - Min Heap
## Binary Heaps Defined
> ![image.png](L1810__Heaps_Priority_Queues.assets/20230324_1601217784.png)



## Requires Complete Trees⭐⭐⭐⭐⭐
> ![image.png](L1810__Heaps_Priority_Queues.assets/20230324_1601212406.png)



# Writing Heap Methods - Min Heap
> ![image.png](L1810__Heaps_Priority_Queues.assets/20230324_1601215718.png)

**Hint**[https://algs4.cs.princeton.edu/24pq/](https://algs4.cs.princeton.edu/24pq/)
[https://algs4.cs.princeton.edu/24pq/MinPQ.java.html](https://algs4.cs.princeton.edu/24pq/MinPQ.java.html)
> 本质上这个`Lab`要实现一个`Minheap`, 最上面的点对应的`Priority`最小, 越下面的节点对应的`Priority`越大。

## swim()
> 就是将一个节点向上冒泡直到他的`parent`比自身要小。由于每个节点只有一个`parent`, 所以简单的递归调用就可以搞定。
...
## sink()⭐⭐⭐⭐⭐
> 🔔: 注意，对于`Minheap`如果在`sink`的过程中碰到两个`children`的值均比当前节点小的情况，我们会选择更小的那个进行交换。
> 注意`inBounds`一定要放在`validateSinkSwimArg`前面，否则`autograder`会不通过, 即便其他逻辑都是对的。很坑。
 ...
 */
private void sink(int index) {
    // 注意inBounds一定要放在validateSinkSwimArg前面，否则autograder会不通过
    if (!inBounds(index)) {
        return;
    }
    ...
    validateSinkSwimArg(index);
    /** TODO: Your code here. */
    // Since the leftNode and rightNode are both smaller than the current Node
    // So here we always pick the node with smaller priority to swap for autograder test.
...
## insert()
> `insert`实现分为两步:
> 1. 首先在`array`的末尾`append`这个元素，`size++`。
> 2. 将这个元素`swim`到指定位置。
...
## peek()
> 比较简单，直接返回`array[1]`, 注意`array[0]`是`null`。
...
## removeMin()
> **比较简单，分为两步:**
> 1. 将`array[1]`和`array[size]`的值交换。
> 2. 将`array[1]`进行`sink`操作到达指定位置。
...
# Submission
> ![image.png](L1810__Heaps_Priority_Queues.assets/20230324_1601216411.png)
